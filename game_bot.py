...
async def process_start_command(message: Message):
    await message.answer(
        ...
        'чтобы начать играть введите да или давай\n\n'
        'Чтобы получить правила игры и список доступных '
        'команд - отправьте команду    /help_new_user'
    )

@dp.message(Command(commands='help_new_user'))
async def process_help_command(message: Message):
    await message.answer(
        f'Правила игры:\n\nЯ загадываю число от 1 до 100, '
        f'а вам нужно его угадать\nУ вас есть {ATTEMPTS} '
        f'попыток, давайте сыграем?'
    )
...
async def process_negativ_answer(message: Message):
    if not users[message.from_user.id]['in_game']:
        ...
        await message.answer('чтобы выйти введите /cancel')
...
async def process_numbers_answer(message: Message):
    if users[message.from_user.id]['in_game']:
        if int(message.text) == users[message.from_user.id]['secret_number']:
            ...
            await message.answer('мы не играем, хотите сыграть?')
